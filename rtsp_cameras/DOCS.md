...
**The icon on the integrations dashboard does not show up**

Home Assistant reads the brand images from `custom_components/rtsp_cameras/brand`, and
the add-on copies its bundled integration into `/config` on start. So both steps
matter:

1. the add-on must be **0.2.2 or newer** (the `brand` folder arrived in 0.2.2), and
2. Home Assistant must be **restarted** after the add-on copied it - the integration
   page shows the loaded version (*Wersja* on `/config/integrations`) and it has to
   read the same version as the add-on. If it shows an older one, press *Install the
   integration* (or *Update*) in the add-on panel, restart Home Assistant and hard
   refresh the browser (Ctrl+F5), because brand images are cached.

The status strip of the add-on panel shows both versions side by side; an amber
`0.1.22 → 0.2.3` chip means the copy inside Home Assistant is still the old one.
